...
pub async fn handle(activity: Activity) -> Result<StatusCode, Error> {
    ...
    let follow_activity_db = crate::database::activity::select::by_url(follow_activity_url).await?;
    ...
    crate::database::activity::update(follow_activity_db.id, follow_activity_value)
        .await
        .unwrap();
