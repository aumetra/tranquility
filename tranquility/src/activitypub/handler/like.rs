...
pub async fn handle(state: &ArcState, activity: Activity) -> Result<StatusCode, Error> {
    ...
    fetcher::fetch_activity(&state, activity_url.as_ref()).await?;
    // Fetch the actor (just in case)
    fetcher::fetch_actor(&state, activity.actor.as_ref()).await?;
    let actor =
        crate::database::actor::select::by_url(&state.db_pool, activity.actor.as_ref()).await?;
